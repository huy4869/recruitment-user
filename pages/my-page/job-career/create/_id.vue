...
import {
  ...
  MASTER_GET_DATA, MY_PAGE_SET_STATE_PAGE,
  WORK_HISTORY_CREATE, WORK_HISTORY_DELETE,
  WORK_HISTORY_DETAIL
...
export default {
  ...
  created() {
    this.$store.commit(INDEX_SET_TITLE_MENU, [
      ...
      { name: this.$t('my_page.job_career') + this.$route.params.id + this.$t('my_page.create'), route: '/my-page/job-career' }
    ])
    this.getMasterData()
  },
  async mounted() {
  ...
  methods: {
    ...
    async getMasterData() {
      const dataResources = [
        ...
        'resources[m_position_offices_user]={}'
      ]
      await this.$store.dispatch(MASTER_GET_DATA, dataResources.join('&')).then(res => {
        if (res.status_code === 200) {
          ...
          this.m_position_offices = res.data.m_position_offices_user.length ? res.data.m_position_offices_user : [{ name: this.$t('career.position_offices'), disabled: true }]
